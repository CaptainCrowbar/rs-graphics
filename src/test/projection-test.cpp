#include "rs-graphics/projection.hpp"
...
namespace {
    ...
    constexpr Double2 zero{0.0, 0.0};
    constexpr Double2 equator{0.0, pi / 2.0};
    constexpr Double2 pcentre{pi / 2.0, pi / 2.0};
    constexpr Double2 pt_north{pi / 8.0, pi / 4.0};
    constexpr Double2 pt_south{pi / 8.0, 3.0 * pi / 4.0};
    constexpr Double2 xy_north{0.1, 0.1};
    constexpr Double2 xy_south{0.1, -0.1};

    const fs::path docs_image_dir{"../docs/images"};
    const fs::path test_image_dir{"../src/test/images"};
    const fs::path earth_map = test_image_dir / "nasa-earth.png";
...
void test_rs_graphics_projection_polar_coordinate_normalisation() {
    ...
    static const std::vector<test_item> test_vector = {
        ...
        { {60_deg, 75_deg},  {0, 90_deg},       {60_deg, 75_deg}  },
        { {0, 30_deg},       {0, 60_deg},       {0, 60_deg}       },
        { {0, 90_deg},       {0, 60_deg},       {0, 120_deg}      },
        { {75_deg, 30_deg},  {75_deg, 60_deg},  {0, 60_deg}       },
        { {75_deg, 90_deg},  {75_deg, 60_deg},  {0, 120_deg}      },
...
void test_rs_graphics_projection_azimuthal_equidistant() {
    ...
    TEST(proj.map_to_globe(xy_north)[1] < 90_deg);
    TEST(proj.map_to_globe(xy_south)[1] > 90_deg);
...
namespace {
    ...
    public:
        ...
        Rgba8 operator()(const Double2& polar) const {
            double x = euclidean_divide(polar[0] + pi, 2 * pi).second;
    ...
    void make_map(std::shared_ptr<BasicMapProjection<double>> proj) {
        ...
        static const std::vector<double> cut_north = {-20_deg};
        static const std::vector<double> cut_south = {-20_deg, 75_deg};
        ...
        auto proj_name = ascii_lowercase(proj->name());
        rs::replace(proj_name, ' ', '-');
        auto map_name = "map-" + proj_name + ".png";
        auto map_path = docs_image_dir / map_name;
        image.save(map_path);

        std::unique_lock lock{mutex};
        std::println("... {}", proj->name());
