...
void test_rs_graphics_noise_statistics() {
    ...
    for (auto oct = 1uz; oct <= max_oct; ++oct) {
        ...
        // "... 2D  oct=" oct "  min=" min "  max=" max "  mean=" mean "  sd=" sd "  sdr=" sdr
    ...
    for (auto oct = 1uz; oct <= max_oct; ++oct) {
        ...
        // "... 3D  oct=" oct "  min=" min "  max=" max "  mean=" mean "  sd=" sd "  sdr=" sdr
    ...
    static const auto make_sample = [] (const std::string& name, auto noise) {
        ...
        for (int j = 0; j < width_pixels; ++j) {
            ...
            for (int i = 0; i < width_pixels; ++i) {
                ...
                auto z8 = static_cast<std::uint8_t>(std::round((z + 1) * 127.5));
                *p++ = Rgba8(z8, 255);
            }
        }

        stbi_write_png(sample_file.data(), width_pixels, width_pixels, 4, pixels.data(), 0);
        std::println("... {}", sample_file);
