...
#include "rs-graphics/stb/stb_image.h"
#include "rs-graphics/stb/stb_image_resize.h"
#include "rs-graphics/stb/stb_image_write.h"
...
namespace RS::Graphics {
    ...
    std::string ImageIoError::make_message(const fs::path& file, const std::string& details, bool stbi) {

        std::string message = "Image I/O error";

        if (! file.empty()) {
            message += std::format(": {:?}", file.string());
        }
    ...
    std::string ImageInfo::str() const {
        ...
        auto s = std::format("Image:{}x{},{}ch@{}bit", shape.x(), shape.y(), channels, bits_per_channel);
    ...
    ImageInfo query_image(const fs::path& file) noexcept {
    ...
    namespace Detail {
        ...
        StbiPtr<Channel> load_image(Function* stb_function, Source* src, size_t len, Int2& shape) {
        ...
        StbiPtr<uint8_t> load_image_8(const fs::path& file, Int2& shape) {
            auto name = file.string();
            return load_image<uint8_t>(&stbi_load, name.data(), 0, shape);
        }

        StbiPtr<uint16_t> load_image_16(const fs::path& file, Int2& shape) {
            auto name = file.string();
            return load_image<uint16_t>(&stbi_load_16, name.data(), 0, shape);
        }

        StbiPtr<float> load_image_hdr(const fs::path& file, Int2& shape) {
            auto name = file.string();
            return load_image<float>(&stbi_loadf, name.data(), 0, shape);
        }

        void save_image_8(const Image<Rgba8>& image, const fs::path& file, const std::string& format, int quality) {
        ...
        void save_image_hdr(const Image<Rgbaf>& image, const fs::path& file) {
            auto name = file.string();
            auto rc = stbi_write_hdr(name.data(), image.width(), image.height(), 4, image.channel_data());
            if (rc == 0) {
                throw ImageIoError(file, {}, false);
            }
        }

        void resize_image_8(const uint8_t* in, Int2 ishape, uint8_t* out, Int2 oshape, int num_channels, int alpha_channel,
                int stb_flags, int stb_edge, int stb_filter, int stb_space) {
            auto rc = stbir_resize_uint8_generic(in, ishape.x(), ishape.y(), 0, out, oshape.x(), oshape.y(), 0,
        ...
        void resize_image_16(const uint16_t* in, Int2 ishape, uint16_t* out, Int2 oshape, int num_channels, int alpha_channel,
                int stb_flags, int stb_edge, int stb_filter, int stb_space) {
            auto rc = stbir_resize_uint16_generic(in, ishape.x(), ishape.y(), 0, out, oshape.x(), oshape.y(), 0,
                num_channels, alpha_channel, stb_flags, stbir_edge(stb_edge), stbir_filter(stb_filter),
                stbir_colorspace(stb_space), nullptr);
            if (rc == 0) {
                throw std::invalid_argument("Internal error: Image resize failed");
        ...
        void resize_image_hdr(const float* in, Int2 ishape, float* out, Int2 oshape, int num_channels, int alpha_channel,
                int stb_flags, int stb_edge, int stb_filter, int stb_space) {
            auto rc = stbir_resize_float_generic(in, ishape.x(), ishape.y(), 0, out, oshape.x(), oshape.y(), 0,
                num_channels, alpha_channel, stb_flags, stbir_edge(stb_edge), stbir_filter(stb_filter),
                stbir_colorspace(stb_space), nullptr);
            if (rc == 0) {
                throw std::invalid_argument("Internal error: Image resize failed");
