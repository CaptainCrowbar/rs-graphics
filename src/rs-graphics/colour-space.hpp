...
namespace RS::Graphics {
    ...
    template <typename T>
    concept ColourChannel = std::floating_point<T> || std::integral<T>;

    namespace Detail {

        template <std::floating_point T>
        constexpr Vector<T, 3> D65{T{0.950489}, T{1}, T{1.088840}};
        ...
        concept ColourSpaceGraphHelper = colour_space_graph_helper<CS>();

        template <Scalar T>
        T fraction(T t) noexcept {
            using std::modf;
            T i_part;
            auto f_part = modf(t, &i_part);
            if (f_part < T{0}) {
                f_part += T{1};
            }
            return f_part;
    ...
    template <typename CS> concept LinearColourSpace = ColourSpace<CS> && (CS::properties & Csp::linear) != Csp::none;
    template <typename CS> concept PolarColourSpace = ColourSpace<CS> && (CS::properties & Csp::polar) != Csp::none;
    template <typename CS> concept RgbColourSpace = ColourSpace<CS> && (CS::properties & Csp::rgb) != Csp::none;
    template <typename CS> concept UnitColourSpace = ColourSpace<CS> && (CS::properties & Csp::unit) != Csp::none;

    // Utility functions

    template <ColourSpace CS, ColourChannel T>
    ...
    constexpr void clamp_colour(Vector<T, CS::count>& colour, T scale = 1) noexcept {
        ...
        if constexpr (PolarColourSpace<CS>) {
            colour[0] = euclidean_divide(colour[0], scale).second;
        }

        if constexpr (UnitColourSpace<CS>) {
            for (auto i = static_cast<std::size_t>(PolarColourSpace<CS>); i < CS::count; ++i) {
    ...
    public:
        ...
        template <ColourChannel T> static constexpr Vector<T, 3> from_base(Vector<T, 3> colour) noexcept { return colour; }
        template <ColourChannel T> static constexpr Vector<T, 3> to_base(Vector<T, 3> colour) noexcept { return colour; }
    ...
    public:
        ...
        template <ColourChannel T> static constexpr Vector<T, 3> from_base(Vector<T, 3> colour) noexcept;
        template <ColourChannel T> static constexpr Vector<T, 3> to_base(Vector<T, 3> colour) noexcept;
    ...
    public:
        ...
        template <ColourChannel T> static Vector<T, 3> from_base(Vector<T, 3> colour) noexcept;
        template <ColourChannel T> static Vector<T, 3> to_base(Vector<T, 3> colour) noexcept;

    private:

        template <ColourChannel T> static constexpr T delta = T(6) / T(29);
        template <ColourChannel T> static constexpr T c1 = delta<T> * delta<T> * delta<T>;
        template <ColourChannel T> static constexpr T c2 = 3 * delta<T> * delta<T>;
        template <ColourChannel T> static constexpr T c3 = T(4) / T(29);

        template <ColourChannel T> static T f(T t) noexcept;
        template <ColourChannel T> static T inverse_f(T t) noexcept;
    ...
    private:
        ...
        template <ColourChannel T> static constexpr T delta = T(6) / T(29);
        template <ColourChannel T> static constexpr T c1 = delta<T> * delta<T> * delta<T>;
        template <ColourChannel T> static constexpr T c2 = T(29) / T(3);
        template <ColourChannel T> static constexpr T c3 = c2<T> * c2<T> * c2<T>;
        template <ColourChannel T> static constexpr T c4 = 1 / c3<T>;

        template <ColourChannel T> static constexpr T u_prime(Vector<T, 3> xyz) noexcept;
        template <ColourChannel T> static constexpr T v_prime(Vector<T, 3> xyz) noexcept;

        template <ColourChannel T> static constexpr T u_prime_n = u_prime(Detail::D65<T>);
        template <ColourChannel T> static constexpr T v_prime_n = v_prime(Detail::D65<T>);
        ...
        Vector<T, 3> CIELuv::to_base(Vector<T, 3> colour) noexcept {
            ...
            Vector<T, 3> out;
            auto u = colour[1] / (13 * colour[0]) + u_prime_n<T>;
            auto v = colour[2] / (13 * colour[0]) + v_prime_n<T>;
    ...
    public:
        ...
        Vector<T, 3> HCLSpace<Base>::from_base(Vector<T, 3> colour) noexcept {
            ...
            auto h = Detail::fraction(chl[1] / (2 * pi_v<T>));
    ...
    template <std::int64_t M00, std::int64_t M01, std::int64_t M02,
        std::int64_t M10, std::int64_t M11, std::int64_t M12,
        std::int64_t M20, std::int64_t M21, std::int64_t M22,
        std::int64_t Divisor>
    class WorkingSpace {
    ...
    public:
        ...
        template <ColourChannel T> static constexpr Vector<T, 3> from_base(Vector<T, 3> colour) noexcept { return xyz_to_rgb_matrix<T> * colour; }
        template <ColourChannel T> static constexpr Vector<T, 3> to_base(Vector<T, 3> colour) noexcept { return rgb_to_xyz_matrix<T> * colour; }

    private:

        template <ColourChannel T> static constexpr auto rgb_to_xyz_matrix = Matrix<T, 3, MatrixLayout::row>
            (T(M00), T(M01), T(M02), T(M10), T(M11), T(M12), T(M20), T(M21), T(M22)) / T(Divisor);
        template <ColourChannel T> static constexpr auto xyz_to_rgb_matrix = rgb_to_xyz_matrix<T>.inverse();

    };

    template <typename WorkingSpace, std::int64_t GammaNumerator, std::int64_t GammaDenominator>
    ...
    public:
        ...
        template <typename WS, std::int64_t GN, std::int64_t GD>
        template <ColourChannel T>
    ...
    namespace Detail {
        ...
        constexpr void rgb_to_hcv(T r, T g, T b, T& h, T& c, T& v) noexcept {
            ...
            h = euclidean_divide(h, T(6)).second / 6;
        ...
        constexpr void hcm_to_rgb(T h, T c, T m, T& r, T& g, T& b) noexcept {
            ...
            h *= 6;
            auto x = c * (1 - std::abs(euclidean_divide(h, T(2)).second - 1));
            r = g = b = m;
    ...
    public:
        ...
        constexpr Vector<T, 3> HSL::from_base(Vector<T, 3> colour) noexcept {
            ...
            if (c != 0) {
                out[1] = c / (T(1) - std::abs(T(2) * out[2] - T(1)));
            }
    ...
    public:
        ...
        template <ColourChannel T> static constexpr Vector<T, 1> from_base(Vector<T, 3> colour) noexcept;
        template <ColourChannel T> static constexpr Vector<T, 3> to_base(Vector<T, 1> colour) noexcept;
    ...
    public:
        ...
        template <ColourChannel T> static constexpr Vector<T, 1> from_base(Vector<T, 1> colour) noexcept;
        template <ColourChannel T> static constexpr Vector<T, 1> to_base(Vector<T, 1> colour) noexcept;

    };

        template <ColourChannel T>
        constexpr Vector<T, 1> sGreyscale::from_base(Vector<T, 1> colour) noexcept {
            colour[0] = Detail::sRGB_function(colour[0]);
            return colour;
        }

        template <ColourChannel T>
        constexpr Vector<T, 1> sGreyscale::to_base(Vector<T, 1> colour) noexcept {
            colour[0] = Detail::sRGB_inverse(colour[0]);
            return colour;
        }

    // Conversion functions

    template <ColourSpace CS1, ColourSpace CS2, ColourChannel T>
