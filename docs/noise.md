...
```c++
using Noise::seed_type = std::uint64_t;
...
```c++
static constexpr std::size_t Noise::dim = N;
```
...
```c++
Noise::Noise();
explicit Noise::Noise(std::uint64_t s) noexcept;
```
...
```c++
void Noise::seed(std::uint64_t s) noexcept;
```
...
```c++
using NoiseSource::seed_type = std::uint64_t;
...
```c++
static constexpr std::size_t NoiseSource::dim_in = DimIn;
static constexpr std::size_t NoiseSource::dim_out = DimOut;
```
...
explicit NoiseSource::NoiseSource(T cell, T scale, std::size_t octaves,
...
```c++
std::size_t NoiseSource::octaves() const noexcept;
void NoiseSource::octaves(std::size_t n) noexcept;
```
...
```c++
void NoiseSource::seed(std::uint64_t s) noexcept;
```
