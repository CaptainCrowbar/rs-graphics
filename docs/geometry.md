...
```c++
template <Scalar T, std::size_t N> class Box;
```
...
```c++
static constexpr std::size_t Box::dim = N;
```
...
constexpr explicit Box::Box(const vector_type& p, const vector_type& v) noexcept;
```
...
Formats a box as a string, in the form `"B[base]+[shape]"`.

## Sphere class

```c++
template <std::floating_point T, std::size_t N> class Sphere;
```
...
```c++
static constexpr std::size_t Sphere::dim = N;
```
...
constexpr explicit Sphere::Sphere(const vector_type& c, T r) noexcept;
```
...
version is true if the sphere `s` is entirely inside `*this` (only points
inside the spheres are considered, so this will still be true if the sphere's
...
```c++
T Sphere::volume() const noexcept;
T Sphere::surface() const noexcept;
```
...
radius by the absolute value of that factor, without moving the centre.
...
```c++
struct std::formatter<Sphere>;
```

Formats a sphere as a string, in the form `"S[centre]+radius"`.
