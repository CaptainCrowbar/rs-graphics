...
```c++
template <ColourChannel VT = float, ColourSpace CS = LinearRGB,
...
using Rgb16 = Colour<std::uint16_t, LinearRGB, ColourLayout::forward>;
...
using sRgb16 = Colour<std::uint16_t, sRGB, ColourLayout::forward>;
...
using Rgba16 = Colour<std::uint16_t, LinearRGB, ColourLayout::forward_alpha>;
...
using sRgba16 = Colour<std::uint16_t, sRGB, ColourLayout::forward_alpha>;
...
```c++
constexpr explicit Colour::Colour(VT x) noexcept;
```
...
```c++
constexpr explicit Colour::Colour(VT x, VT a) noexcept;
```
...
```c++
constexpr explicit Colour::Colour(vector_type v) noexcept;
```
...
```c++
template <typename... Args> constexpr explicit Colour::Colour(Args... args) noexcept;
```
...
type but not in the string, it will be set to full opacity. If an alpha
channel is present in the string but not the type, it will be ignored.
...
```c++
VT& Colour::operator[](std::size_t i) noexcept;
const VT& Colour::operator[](std::size_t i) const noexcept;
```
...
```c++
constexpr VT& Colour::cs(std::size_t i) noexcept;
constexpr const VT& Colour::cs(std::size_t i) const noexcept;
```
...
```c++
std::size_t Colour::hash() const noexcept;
...
nonsense if the colour is out of gamut. This is only defined for RGB colour
spaces.
...
```c++
constexpr std::size_t Colour::size() const noexcept;
```

Returns the number of channels (equal to the `channels` constant).

```c++
struct std::formatter<Colour>;
```
...
This converts between colours in any two colour spaces. This will call
`convert_colour_space()` if the colour spaces are different, as well as
converting between different channel types and internal layouts. Conversions
from floating point to integer will round to the nearest integer; halves
round upwards.

If the output contains an alpha channel but the input does not, the alpha
channel will be set to its fully opaque value. If the input contains an alpha
channel but the output does not, the alpha channel will be discarded.
