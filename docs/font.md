...
```c++
explicit Font::Font(const std::filesystem::path& file, int index = 0);
```
...
```c++
static std::vector<Font> Font::load(const std::filesystem::path& file);
```
...
template <LinearColourType C, int F>
    void ScaledFont::render(Image<C, F>& image, Int2& offset,
...
```c++
template <LinearColourType C, int F>
    void ScaledFont::render_to(Image<C, F>& image, Int2 ref_point,
        const std::string& text, int line_shift = 0,
...
```c++
std::size_t ScaledFont::text_fit(const std::string& text,
...
```c++
std::size_t ScaledFont::text_wrap(const std::string& text_in,
    std::string& text_out, std::size_t max_pixels) const;
```
...
in a given set of directories. Fonts are organised by family and
subfamily (the subfamily usually represents variations such as bold or
italic).
...
std::vector<std::string>
    FontMap::subfamilies(const std::string& family) const;
```
...
```c++
void FontMap::search(const std::string& dir, bool recurse = false);
```

Searches the given directory for all readable font files. This will do nothing
if `dir` does not exist or is not a directory. Searches are cumulative;
previously loaded fonts are not cleared before a search. Subdirectories are
searched recursively if the `recurse` flag is set.
...
```c++
std::size_t FontMap::size() const noexcept;
bool FontMap::empty() const noexcept;
```

Check the number of font families in the `FontMap`.
