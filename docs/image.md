...
```c++
enum class ImageFlags: std::uint8_t {
    none    = 0,
    invert  = 1,
    pma     = 2,
}
...
```c++
enum class ImageResize: std::uint8_t {
    none    = 0,
    unlock  = 1,
    wrap    = 2,
}
...
`operator++()`, or `pos()` are called on an off-image iterator.
...
```c++
explicit Image::Image(Int2 shape);
Image::Image(Int2 shape, CT c);
Image::Image(int w, int h);
Image::Image(int w, int h, CT c);
```

Create an image with the specified dimensions. If a colour is supplied, the
new image will be filled with that colour; otherwise, the pixel data is
initialized to zero. These will throw `std::invalid_argument` if either
...
```c++
CT& Image::operator[](Int2 p) noexcept;
const CT& Image::operator[](Int2 p) const noexcept;
...
```c++
iterator Image::locate(Int2 p) noexcept;
const_iterator Image::locate(Int2 p) const noexcept;
...
```c++
void Image::save(const std::filesystem::path& file, int quality = 90) const;
```
...
```c++
void Image::reset(Int2 new_shape);
void Image::reset(Int2 new_shape, CT c);
...
```c++
void Image::resize(Int2 new_shape,
    ImageResize flags = ImageResize::none);
void Image::resize(double scale,
    ImageResize flags = ImageResize::none);
Image Image::resized(Int2 new_shape,
...
Int2 Image::shape() const noexcept;
